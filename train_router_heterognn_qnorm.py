"""
NEW FILE: train_router_heterognn_qnorm.py

joint prompt--model router heterogeneous GNN router with text-description node features and
query-normalized rewards.

This is the paper-consistent replacement for train_router_bipartite_qnorm.py:
  - graph node types: task, query, prompt, model
  - node initialization:
      query  <- query text embeddings
      task   <- task description embeddings
      prompt <- prompting-strategy description embeddings
      model  <- model description embeddings
  - message passing over the heterogeneous graph
  - action scoring: s(q,p,m) = MLP([h_q ; h_p ; h_m])

Leakage rule:
  performance, cost, and reward values are NEVER used as node features or
  message-passing edge attributes. They are used only as supervised labels and
...
# Canonical order for the paper setup.
TASK_TO_ID = {
    "gsm8k": 0,
    "hotpotqa": 1,
    "squad": 2,
...
def load_query_embeddings(path: Path) -> Dict[str, torch.Tensor]:
    obj = torch.load(path, map_location="cpu")
    if isinstance(obj, dict) and "embedding_by_id" in obj:
        return {str(k): v.float() for k, v in obj["embedding_by_id"].items()}
    if isinstance(obj, dict):
        return {str(k): v.float() for k, v in obj.items()}
    raise TypeError(f"Unsupported query embedding format in {path}")


def load_node_embedding_matrix(path: Path, expected_to_id: Dict[str, int], node_type: str) -> torch.Tensor:
    """Load embeddings saved by scripts/build_node_description_embeddings.py.

    Also accepts a raw dict id -> tensor. Returns matrix ordered by expected_to_id.
    """
    obj = torch.load(path, map_location="cpu")

    if isinstance(obj, dict) and "embedding_by_id" in obj:
        emb_by_id = obj["embedding_by_id"]
    elif isinstance(obj, dict):
        emb_by_id = obj
    else:
        raise TypeError(f"Unsupported {node_type} embedding format in {path}")

    missing = [k for k in expected_to_id if k not in emb_by_id]
    if missing:
        raise KeyError(f"Missing {node_type} embeddings for ids: {missing}. File: {path}")

    matrix = [emb_by_id[k].float().view(-1) for k, _ in sorted(expected_to_id.items(), key=lambda kv: kv[1])]
...
@dataclass
class StaticNodeFeatures:
    task: torch.Tensor
    prompt: torch.Tensor
    model: torch.Tensor

    def to(self, device: torch.device) -> "StaticNodeFeatures":
        return StaticNodeFeatures(
...
def build_graph_for_qids(
    qids: set,
    router_queries: List[dict],
    query_embs: Dict[str, torch.Tensor],
    static_features: StaticNodeFeatures,
) -> HeteroGraphBatch:
    """Build a structural graph for selected queries.

    Edges represent available structure only: task membership, candidate prompt/model
    availability, and the prompt-model action lattice. Labels are not edge features.
    """
    selected_rows = [
        r for r in router_queries if r["qid"] in qids and r["qid"] in query_embs and r["task"] in TASK_TO_ID
    ]
    ...
    return HeteroGraphBatch(
        x_dict={
            "query": query_x,
            "task": static_features.task,
            "prompt": static_features.prompt,
            "model": static_features.model,
        },
        edge_index_dict={
...
class HeteroSageLayer(nn.Module):
    """Dependency-free heterogeneous GraphSAGE-style message passing layer."""
...
def train_one_lambda(
    ...
    train_graph = build_graph_for_qids(train_qids, router_queries, query_embs, static_features).to(device)
    val_graph = build_graph_for_qids(train_qids | val_qids, router_queries, query_embs, static_features).to(device)
    test_graph = build_graph_for_qids(train_qids | val_qids | test_qids, router_queries, query_embs, static_features).to(device)
    train_data = tensorize_examples(train_ex, train_graph, device)

    query_dim = next(iter(query_embs.values())).numel()
    model = GraphRouterPPHeteroGNN(
        query_dim=query_dim,
        task_dim=static_features.task.shape[1],
        prompt_dim=static_features.prompt.shape[1],
        model_dim=static_features.model.shape[1],
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for idx in minibatch_indices(len(train_ex), args.batch_size, device, shuffle=True):
            # Re-encode per minibatch to avoid retaining stale computation graphs after optimizer.step().
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--task-emb-path", type=Path, default=DEFAULT_TASK_EMB_PATH)
    parser.add_argument("--prompt-emb-path", type=Path, default=DEFAULT_PROMPT_EMB_PATH)
    parser.add_argument("--model-emb-path", type=Path, default=DEFAULT_MODEL_EMB_PATH)
...
def main() -> None:
    ...
    query_embs = load_query_embeddings(args.query_emb_path)
    static_features = StaticNodeFeatures(
        task=load_node_embedding_matrix(args.task_emb_path, TASK_TO_ID, "task"),
        prompt=load_node_embedding_matrix(args.prompt_emb_path, PROMPT_TO_ID, "prompt"),
        model=load_node_embedding_matrix(args.model_emb_path, MODEL_TO_ID, "model"),
    )

    all_results = []
    for lambda_key, lam in LAMBDA_CONFIGS:
        set_seed(args.seed)
        result = train_one_lambda(lambda_key, lam, router_queries, query_embs, static_features, args, device)
